...
const adminLogin=function(req,res){
 ...
 try{
    ...
    res.render('admin/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while admin login.",link:req.headers.referer || '/admin'})
...
const adminLoginValidation=async function(req,res,next){ 
  ...
  console.log("=========== " + adminvalidation.Password + '===='+ adminvalidation.Email)
  ...
  try{
    ...
    if(adminvalidate && adminvalidate.isAdmin){ 
      ...
      const password=await bcrypt.compare(adminvalidation.Password,adminvalidate.Password)
      if(password){
    ...
    res.render('admin/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while login validation",link:req.headers.referer || '/admin'})
...
const dashBoard=async function(req,res){
  try {
    ...
    if(sessionName){
      ...
      res.render('admin/adminHome',{sessionName,latestMembers,latesOrders})
    }else{
      res.redirect('/admin')
    }
  } catch (error) {
    console.log('error in admin dashboard', error.message)
    res.render('admin/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while loading dashboard",link:req.headers.referer || '/admin'})
...
const productList=async function(req,res){
  ...
  const products = await Products.find({}).populate('Category').sort({CreatedAt:-1}).skip(skip).limit(limit)
  const totalProducts = await Products.countDocuments();
  const totalPages=Math.ceil(totalProducts / limit)
  if(sessionName){
  res.render('admin/productList',{sessionName,products,totalPages,currentPage:currentPage})
  }else{
    res.redirect('/admin/')
  }
}
...
const userListget= async function(req,res){
     ...
     const allUsers=await Users.find({}).sort({CreatedAt:-1}).skip(skip).limit(limit)
     const totalUsers=await Users.countDocuments()
     const totalPages=Math.ceil(totalUsers / limit)
     if(sessionName){
      res.render('admin/userList',{sessionName,allUsers,totalPages,currentPage})
     }else{
      res.redirect('/admin/')
     }
}
...
const blockUser =async  function(req,res){
     ...
     const user= await Users.updateOne({_id:user_id},{$set:{Status:false}})
     console.log(user)
...
const unblockUser =async function(req,res){
  const user_id=req.params.id
  const user= await Users.updateOne({_id:user_id},{$set:{Status:true}})
  console.log(user)
  res.redirect('/admin/userlist')
}
...
const orderListget=async function(req,res){
  try {
    ...
    let message;
    let allOrders=await Ordercoll.find({}).populate('orderedUser')
...
const updateOrderStatus=async function(req,res){
  try {
    ...
    if(currentOrderStatus.orderStatus=='Delivered'){
      return res.status(400).json({ error: 'Cannot update delivered order status' });
    }

    let order=await Ordercoll.findByIdAndUpdate(order_id,{$set:{orderStatus:status}},{new:true})
    console.log(order)
...
const coupenListget=async function(req,res){
  ...
  if(sessionName){
    res.render('admin/coupenList',{sessionName,allCoupens})
  }else{
    return res.redirect('/admin/')
  }
}

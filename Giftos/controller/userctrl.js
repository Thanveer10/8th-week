...
const homepage=async function(req,res){
 try {
  ...
  let latestProducts=await ProductColl.find({CreatedAt:{ $gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) }}).populate('Category').sort({CreatedAt:-1}).limit(10)
  latestProducts= latestProducts.filter((product)=>product.Category&&product.Category.Status=="Listed")
   if(user){
    ...
    return res.render('user/index',{sessionName,latestProducts,userdata})
   }
  ...
  res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while trying to load the homepage.",link:req.headers.referer || '/'})
 }
...
const shop=async function(req,res){
  try {
    ...
    const user=req.session.user_id
    let latestProducts=await ProductColl.find({CreatedAt:{ $gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) }}).sort({CreatedAt:-1}).skip(skip).limit(limit)
    let totalProducts= await ProductColl.countDocuments({CreatedAt:{ $gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) }})
    let totalPages=Math.ceil(totalProducts / limit)
    if(user){
      let   sessionName=user
      let userdata=await User.findOne({_id:user})
      res.render('user/shop',{sessionName,latestProducts,userdata,currentPage,totalPages})
    }else{
      res.render('user/shop',{latestProducts,currentPage,totalPages})
    }
  } catch (error) {
    console.log('error in shoe section' + error.message)
    res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while trying to load the shoap.",link:req.headers.referer || '/'})
  }
...
const contactpage=function(req,res){
  const user=req.session.user_id
  sessionName=user
  res.render('user/contact',{sessionName})
}
...
const whyus=async function(req,res){
  try {
    ...
    res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while trying to load the why us.",link:req.headers.referer || '/'})
...
const categoryget=async function(req,res){
  ...
  res.render('user/categoryProducts', { products, categoryName,allcategory });
  } catch (error) {
    console.log('erron in categoryget'+ error.message)
    res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while trying to load the Catogrypage.",link:req.headers.referer || '/products/viewAllProducts'})
  }
...
const search=async function(req,res){
  try {
    ...
    const products = await ProductColl.find({  Productname: { $regex: startsWithRegex } });
    console.log(products)
    res.render('user/allProducts', { products,allcategory,sessionName });
  } catch (error) {
    console.log('error in search'+ error.message)
    res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while trying to load the page.",link:req.headers.referer || '/'})
  }
...
const loginpage=function(req,res,next){
  try {
   ...
   res.render('user/userLogin',{err,message})
    ...
    res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while trying to load the loginpage.",link:req.headers.referer || '/'})
...
const signup=function(req,res){
  try {
    ...
    res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while trying to load the signup page.",link:req.headers.referer || '/'})
...
const otplogin=function(req,res){
  try {
    ...
    res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while trying to load the otplogin.",link:req.headers.referer || '/'})
    ...
    if (checkuser) {
      ...
      if (checkuser.Status) {
        ...
        let details = {
          ...
          text: genOtp + " is your GIFTOS verification code. Do not share OTP with anyone "
        }
...
const loginvalidation=async function (req,res){
  ...
  try{
    ...
    if(userData && userData.Status &&isMatch){
      ...
      res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred in login validation.",link:req.headers.referer || '/'})
    }
}
...
const frgtchangePasswordpost=async function(req, res){
  try {
    ...
    res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:error.message,link:req.headers.referer || '/'})
  ...
  try {
    ...
    if(user_id){
      ...
      if(userCart && userCart.Product){
         cartProductIds=userCart.Product.map((product)=>product.item.toString())
    ...
    res.render('user/productDetails',{sessionName,product,related_products,stockerr,isCartItem})
    req.session.prodid=null
    stockerr=undefined 
  } catch (error) {
    console.log('erron in prouctdetailsget' + error.message)
    res.render('user/error',{res,errorCode:500,errorMessage:error.message,errorDescription:'somthing wrong in getting product details',link:req.headers.referer || '/'}) 
  }
...
const addtoCart = async function (req, res, next) {
  ...
  //  }
   ...
   return res.redirect(`/productdetails/${prodId}`)
    ...
    if (userCart) {
      ...
      if (proExist) {
          ...
          if(totalQuantity>=product.MaxPerPerson){
            stockerr='Sorry, you can only add a maximum of '+ product.MaxPerPerson+' items in your cart at a time'
            return res.redirect(`/productdetails/${prodId}`)
          }
  
          await cartColl.updateOne(
            { UserId: userId, "Product.item": new mongoose.Types.ObjectId(prodId) },
            { $inc: { 'Product.$.quantity': 1 } }
          );
        }
      } else {
        await cartColl.updateOne({ UserId: userId }, { $push: { Product: proObj } });
      }
    } else {
      const cartObj = {
        UserId: userId,
    ...
    console.log('error occured while addo cart',error ,'=======', error.message);
    res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while add to cart.",link:req.headers.referer || '/'})
...
const userCart = async function (req, res, next) {
  try {
    ...
    for (let i = 0; i < productCart.length; i++) {
      if (productCart[i].Product && (productCart[i].Product.SalePrice || productCart[i].Product.RegularPrice)) {
        const totalPrice = productCart[i].quantity * (productCart[i].Product.SalePrice  || productCart[i].Product.RegularPrice);
     ...
     grand_total = productCart.reduce((sum, cart) => sum + (cart.totalPrice || 0), 0);
     totalQuantity = productCart.reduce((sum, cart) => sum + (cart.quantity || 0), 0);
  
    // ==========copuen session================================
  let coupen= req.session.coupen
  if(coupen) {
    console.log(
      'dlskdlfkslfdsf  '+ coupen
    );
    ...
    if(grand_total<value.MinimumPrice){
      ...
      coupenerr='minimum prise not reached plesae purchase upto '+value.MinimumPrice
    }else{
      ...
      if(cart){
        const result=await cartColl.updateOne({UserId: user},{$set:{discountPrice:value.DiscountPrice}})
        // cart.discountPrice= value.DiscountPrice
        console.log('discount price added to cart'+ cart.discountPrice)
        console.log(result)
      }
      await User.updateOne({_id:user}, {$addToSet:{UsedCoupen:coupen}})
   
    }
    
  }
    res.render('user/cart', { productCart, grand_total, totalQuantity,sessionName,coupenerr,success,discPrice });
    coupen=null
    ...
    console.log('an error occurred while user cart', error, '=====', error.message)
    res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while looking usercart.",link:req.headers.referer || '/'})
...
///update cart=======================================
  const updateCart= async (req, res) => {
    ...
    try {
      ...
      const updateResult = await cartColl.updateOne(
        { UserId: userId, "Product.item": new mongoose.Types.ObjectId(productId) },
      ...
      const userCart = await cartColl.aggregate([
        ...
        },
        {
          $project: {
            ...
            productPrice: {
              $cond: {
                if: { $or: [ { $eq: ["$productDetails.SalePrice", null] }, { $eq: ["$productDetails.SalePrice", []] } ] },
      ...
      const updatedProduct = userCart.find(item => item.item.toString() === productId);
      console.log("upadate product " + updatedProduct)
  ...
  const deleteCart=async function(req,res){
    ...
    const deletedItem=await cartColl.updateOne({UserId:User},{$pull:{Product:{item: product_id}}})
    console.log(deletedItem)
    res.redirect('/adduserCart');
    } catch (error) {
      console.log('an error occured in delete cart', error.message)
      res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while deleting cart.",link:req.headers.referer || '/'})
    }
 ...
 const coupenApply=async function(req,res){
  try {
    ...
    if(checkcoupen){
    let usedcoupen=await User.findOne({_id:userId,UsedCoupen:{$in:[coupenCode]}})
    ...
    res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while applying copen.",link:req.headers.referer || '/adduserCart'})
...
const checkoutget=async function (req,res,next) {
    ...
    try {
     ...
     let sessionName = userId;
      ...
      if (userId) { 
        let userAddress= await AddressColl.findOne({ UserId: req.session.user_id })
        ...
        try {
          cart = await cartColl.findOne({ UserId: userId }).populate('Product.item');

          if (cart && cart.Product.length > 0) {
            for (let i = 0; i < cart.Product.length; i++) {
              const item = cart.Product[i];
              console.log('product item in checkout get' + item + ' ===' + item.id)
              try {
                ...
                console.error(`Error fetching product for item ${item.item_id}:`, productError);
      ...
      res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while checkout.",link:req.headers.referer || '/adduserCart'})
...
const wishListPost = async function (req, res, next) {
  ...
  if (sessionName == null) {
    ...
    if (userCart) {
      let proExist = await wishListColl.findOne({ $and: [{ UserId: userId }, { "Product.item": new mongoose.Types.ObjectId(proId) }] })
      console.log(proExist);

      if (proExist != null && proExist != "") {
        await wishListColl.updateOne({ "Product.item": new mongoose.Types.ObjectId(proId)}, { $inc: { 'Product.$.quantity': 1 } })
      }
      else {
        await wishListColl.updateOne({ UserId: userId }, { $push: { Product: proObj } })
      }
    }
    else {
      let wishObj = {
  ...
  const wishDelete=async function(req,res){
    ...
    const product_id =req.params.id
    console.log(product_id)
    const deletedItem=await wishListColl.updateOne({UserId:User},{$pull:{Product:{item: new mongoose.Types.ObjectId(product_id)}}})
    console.log(deletedItem)
    res.redirect('/wishlist');
  }
...
let passerrmsg ;
  ...
  const editPasswordPost = async function (req, res, next) {
    try {
      ...
      if (isMatch) {
        console.log(`user password in update password ${user.Password} and ${CurrentPassword}`);
      ...
      res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred edit passowrd.",link:req.headers.referer || '/user-profile'})
    }
   ...
   const userAddressget=async function(req,res){
    ...
    try {
      ...
      if (userId) {
        ...
        res.render("user/userAddress", {user, userAddress,sessionName,totalPages,currentPage});
        }else{
          console.log('user dont have address')
          res.render("user/userAddress", { userAddress,sessionName,totalPages,currentPage})
        }
      } else {
        res.redirect("/login");
      }
      ...
      res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while getting address.",link:req.headers.referer || '/user-profile'})
    }
  ...
  const addAddressget=async function(req,res){
    try {
      ...
      res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while fetching page.",link:req.headers.referer || '/user-profile'})
    }
  ...
  const addAddresspost = async function(req,res){
    try {
      ...
      const {name,houseName,street, city, state, pincode, phone,altPhone,redirectPath} = req.body
    ...
    const phonePattern = /^\d{10}$/;
      ...
      if(!address){
        const newAddress=new AddressColl({
          UserId:user._id,
          Address:[{name:name,houseName:houseName ,street:street, city:city, state:state,pincode:pincode,phone:phone,altPhone:altPhone}]
        })
        await newAddress.save()
        console.log('created new address instance')
      }else{
        address.Address.push({name,houseName,street,city,state,pincode,phone,altPhone})
        await address.save()
  ...
  const editAddressget= async function(req, res, next) {
    try {
      ...
      if(!editAddress){
       ...
       res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred in fetching edit address page.",link:req.headers.referer || '/user-profile'})
      }
  ...
  const editAddresspost=async function(req,res){
    try {
      ...
      if (!name || !houseName || !street || !city || !state || !pincode || !phone) {
        ...
        return res.status(400).redirect(`/edit-address/${addressId}?message=${encodeURIComponent(message)}`);
        }
    }
    ...
    if (!phonePattern.test(phone)) {
        ...
        if (redirectPath === '/checkout') {
            return res.redirect(`/checkoutaddressMessage=${encodeURIComponent(message)}`);
        }else{
        return res.status(400).redirect(`/editAddress/${addressId}?message=${encodeURIComponent(message)}`);
        }
    }
    ...
    if (altPhone && !phonePattern.test(altPhone)) {
      ...
      if (redirectPath === '/checkout') {
        return res.redirect(`/checkout?addressMessage=${encodeURIComponent(successMessage)}`);
    } else {
    ...
    res.redirect('/user-address')
      ...
      res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred while editing  address.",link:req.headers.referer || '/user-profile'})
    }
  }
    ...
    try {
      console.log('reached confirm order')
        ...
        for (let item of cart.Product) {
            ...
            totalCartPrice += (product.SalePrice || product.RegularPrice) * item.quantity;
        }
        ...
        if (!userAddress.length) 
            return res.status(404).redirect(`/checkout?cartMessage=Address not found.`);
        ...
        if (paymentMethod === "Cash on Delivery") {
          let productsArray=[]
            ...
            const order = new OrderColl({
              ...
              date: new Date(),
              discount: cart.discountPrice, // Total discount applied
              grandTotal: totalCartPrice - cart.discountPrice, // Calculate grand total
            ...
            const TotalPrice=totalCartPrice-cart.discountPrice   
            await cartColl.findByIdAndDelete(cartId);
            return res.status(200).json({ CODsuccess: true, cartId : cart._id ,TotalPrice});

            // return res.status(200).json({ success: true, TotalPrice: totalCartPrice - cart.discountPrice });
        // } else if (paymentMethod === "Online") {
        //     const razorpayOrder = await razorpayInstance.orders.create({
        //         amount: (totalCartPrice - discountAmount) * 100,
        //         currency: "INR",
        //     });
...
const LoadOrderPage = async (req, res) => {
  try {
      ...
      const userOrders= await OrderColl.find({orderedUser: user})
      ...
      res.render('user/error',{res,errorCode:500,errorMessage:'Server Error',errorDescription:"An unexpected error occurred load order page.",link:req.headers.referer || '/'})
  }
};
...
const cancelOrder = async (req, res) => {
  try {
    ...
    const updateOrder= await OrderColl.findByIdAndUpdate(
      ...
      {$set:{ "orderStatus": 'Cancelled',cancellationDate: new Date()}},{new:true}
    )
    ...
    for(const product of order.products){
      const updatePorductResult= await ProductColl.updateOne({_id:product.productId}, {$inc:{Stock :product.quantity}})
    ...
    return res.status(200).json({ success: true, message: 'Order cancelled !' });
      ...
      console.error("Error canceling order session: ", error.message);
      return res.status(500).json({ success: false, message: "Failed to cancel order session." });
  }
};
  //logout session
